...
export default function NavBar({image}){
    ...
    return(
        <nav className='flex justify-around items-center mt-4 border-b-2 border-l-slate-900 shadow'>
            <button onClick={() => router.push("/")} className="text-lg font-bold"> explore </button>
            <button onClick={() => router.push("/map")} className=" text-lg font-bold " href="/map"> map </button>
            <button onClick={() => router.push("/")} className='w-1/4'>
                <img className='w-full' src="/travelgenieLogo.svg" alt="travel genie Logo"/>
            </button>
            <button onClick={() => router.push("/createpost")} className="text-lg font-bold"> create post </button>
            <a>
            
            <img onClick={() => {showProfile(!profile)}} className="text-lg font-bold w-8" src={session? session.user.image: "/favicon.ico"}/>
            {
